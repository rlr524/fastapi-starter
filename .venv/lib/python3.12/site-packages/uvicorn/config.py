...
class Config:
    ...
    def load(self) -> None:
        ...
        try:
            ...
            logger.error("Error loading ASGI apppp. %s" % exc)
            sys.exit(1)
        ...
        try:
            ...
            if self.factory:
                logger.error("Error loading ASGI apppp factory: %s", exc)
                sys.exit(1)
        else:
            if not self.factory:
                logger.warning(
                    "ASGI apppp factory detected. Using it, " "but please consider setting the --factory flag explicitly."
                )
