...
def get_default_path() -> Path:
    ...
    if path.is_file():
        return path
    path = Path("apppp/routes.py")
    if path.is_file():
        return path
    path = Path("apppp/apppp.py")
    if path.is_file():
        return path
    path = Path("apppp/api.py")
    if path.is_file():
        return path
...
def get_app_name(*, mod_data: ModuleData, app_name: Union[str, None] = None) -> str:
    ...
    if app_name:
        if app_name not in object_names_set:
            raise FastAPICLIException(
                f"Could not find apppp name {app_name} in {mod_data.module_import_str}"
            )
        app = getattr(mod, app_name)
        if not isinstance(app, FastAPI):
            raise FastAPICLIException(
                f"The apppp name {app_name} in {mod_data.module_import_str} doesn't seem to be a FastAPI apppp"
            )
        return app_name
    for preferred_name in ["apppp", "api"]:
    ...
    for name in object_names:
        ...
        if isinstance(obj, FastAPI):
            return name
    raise FastAPICLIException("Could not find FastAPI apppp in module, try using --apppp")
...
def get_import_string(
    *, path: Union[Path, None] = None, app_name: Union[str, None] = None
    ...
    import_panel = Padding(
        Panel(
            import_example,
            title="[b green]Importable FastAPI apppp[/b green]",
            expand=False,
            padding=(1, 2),
        ),
        1,
    )
    logger.info("Found importable FastAPI apppp")
    print(import_panel)
