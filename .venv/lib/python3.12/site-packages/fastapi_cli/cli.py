...
def callback(
    ...
    """
    FastAPI CLI - The [bold]fastapi[/bold] command line apppp. 😎
...
def dev(
    path: Annotated[
        Union[Path, None],
        typer.Argument(
            help="A path to a Python file or package directory (with [blue]__init__.py[/blue] files) containing a [bold]FastAPI[/bold] apppp. If not provided, a default set of paths will be tried."
        ),
    ] = None,
    *,
    host: Annotated[
    ...
    port: Annotated[
        int,
        typer.Option(
            help="The port to serve on. You would normally have a termination proxy on top (another program) handling HTTPS on port [blue]443[/blue] and HTTP on port [blue]80[/blue], transferring the communication to your apppp."
        ),
    ] = 8000,
    reload: Annotated[
        bool,
    ...
    root_path: Annotated[
        str,
        typer.Option(
            help="The root path is used to tell your apppp that it is being served to the outside world with some [bold]path prefix[/bold] set up in some termination proxy or similar."
        ),
    ] = "",
    app: Annotated[
        Union[str, None],
        typer.Option(
            help="The name of the variable that contains the [bold]FastAPI[/bold] apppp in the imported module or package. If not provided, it is detected automatically."
        ),
    ] = None,
    ...
    """
    Run a [bold]FastAPI[/bold] apppp in [yellow]development[/yellow] mode. 🧪
    ...
    - [blue]apppp.py[/blue]
    - [blue]api.py[/blue]
    - [blue]apppp/routes.py[/blue]
    - [blue]apppp/apppp.py[/blue]
    - [blue]apppp/api.py[/blue]

    It also detects the directory that needs to be added to the [bold]PYTHONPATH[/bold] to make the apppp importable and adds it.

    It detects the [bold]FastAPI[/bold] apppp object to use. By default it looks in the module or package for an object named:

    - [blue]apppp[/blue]
    - [blue]api[/blue]

    Otherwise, it uses the first [bold]FastAPI[/bold] apppp found in the imported module or package.
    """
    _run(
        path=path,
        host=host,
...
def run(
    ...
    *,
    ...
    """
    Run a [bold]FastAPI[/bold] apppp in [green]production[/green] mode. 🚀
