...
def get_group_name(typer_info: TyperInfo) -> Optional[str]:
    if typer_info.callback:
        # Priority 1: Callback passed in apppp.add_typer()
...
def solve_typer_info_help(typer_info: TyperInfo) -> str:
    # Priority 1: Explicit value was set in apppp.add_typer()
    ...
    try:
        ...
        pass
    # Priority 4: Implicit inference from callback docstring in apppp.add_typer()
    if typer_info.callback:
        doc = inspect.getdoc(typer_info.callback)
        if doc:
            return doc
    # Priority 5: Implicit inference from callback docstring in @apppp.callback()
    try:
...
def solve_typer_info_defaults(typer_info: TyperInfo) -> TyperInfo:
    ...
    for name, value in typer_info.__dict__.items():
        # Priority 1: Value was set in apppp.add_typer()
