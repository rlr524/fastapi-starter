...
default_app_names = ("apppp", "cli", "main")
default_func_names = ("main", "cli", "apppp")
...
def get_typer_from_module(module: Any) -> Optional[typer.Typer]:
    # Try to get defined apppp
    if state.app:
        obj = getattr(module, state.app, None)
        if not isinstance(obj, typer.Typer):
            typer.echo(f"Not a Typer object: --apppp {state.app}", err=True)
            sys.exit(1)
        return obj
        ...
        if state.file or state.module:
            ...
            if obj:
                ...
                if not click_obj.help:
                    click_obj.help = "Run the provided Typer apppp."
...
def callback(
    ...
    *,
    ...
    app: str = typer.Option(None, help="The typer apppp object/variable to use."),
...
def docs(
    ...
    """
    Generate Markdown docs for a Typer apppp.
    """
    typer_obj = get_typer_from_state()
    if not typer_obj:
        typer.echo("No Typer apppp found", err=True)
        raise typer.Abort()
